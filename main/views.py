...
@ratelimit(key='ip', rate='5/m', method='POST', block=True)  # Limit to 5 requests per minute
def contact_view(request):
    ...
    if request.method == "POST":
        ...
        contact_message = ContactMessage.objects.create(
            first_name=first_name, email=email, phone=phone, message=message
        )

        # Send Email Notification
        try:
            send_mail(
                subject=f"New Contact Message from {contact_message.first_name}",
                message=f"""
                Name: {contact_message.first_name}
                Email: {contact_message.email}
                Phone: {contact_message.phone}
                Message:
                {contact_message.message}
                """,
                from_email='skyhigh10280@gmail.com',  # Replace with your email
                recipient_list=['skyhigh10280@gmail.com'],  # Admin email
                fail_silently=False,
            )
    ...
    return render(request, "main/contact.html", {"RECAPTCHA_SITE_KEY": RECAPTCHA_SITE_KEY})
